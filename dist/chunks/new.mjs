...
async function main(args) {
  ...
  if (framework.value === "nest") {
    ...
    writeJson(resolve$1(name.value, "nest-cli.json"), nestConfig);
    let tsConfig = readJson(resolve$1(name.value, "tsconfig.json"));
    if (tsConfig.compilerOptions === void 0) {
      tsConfig.compilerOptions = {};
    }
    if (tsConfig.compilerOptions.paths === void 0) {
      tsConfig.compilerOptions.paths = {};
    }
    tsConfig.compilerOptions.paths["~/*"] = ["src/*"];
    tsConfig.compilerOptions.strict = true;
    tsConfig.compilerOptions.strictNullChecks = true;
    writeJson(resolve$1(name.value, "tsconfig.json"), tsConfig);
    console.log("tsconfig.json created.\n");
    const prettierConfig = readJson(resolve$1(name.value, ".prettierrc"));
    prettierConfig.semi = false;
    console.log(".prettierrc created.\n");
    writeJson(resolve$1(name.value, ".prettierrc"), prettierConfig);
    ...
    for (const file of disposedFiles) {
      ...
      rimrafSync(resolve$1(name.value, file));
    }
  ...
  if (framework.value === "nuxt") {
    ...
    for (const _module of _modules) {
      await asyncSpawn(shell, ["-c", npm.execute, npm.name === "bun" ? "--bun" : void 0, "nuxi@latest", "module", "add", _module].filter((item) => item));
    }
    console.log("\nCreating files...\n");
    console.log("package.json created.\n");
    writeJson(resolve$1(name.value, "package.json"), packageJson);
...
`
    ...
    console.log("\nUpdating Nuxt...\n");
    await asyncSpawn(shell, ["-c", npm.execute, npm.name === "bun" ? "--bun" : void 0, "nuxi@latest", "upgrade", "--force"].filter((item) => item));
    console.log("Updating dependencies...\n");
  }
  console.log("\nEnjoy your new application! \u{1F525}");
}
const _new = defineCommand({
  meta: {
    name: "new",
